...
import { TMP_MARKET_ID, usdcAddresses, usdcDecimals } from '../constants'
...
const getSpenderAllowanceUsd = async (networkSelected: LedgerId, smartContractId: string, accountId: string): Promise<number> => {
  try {
    const networkKey = networkSelected.toString().toUpperCase() as keyof typeof usdcAddresses
    const mirrornode = `https://${networkSelected}.mirrornode.hedera.com/api/v1/accounts/${accountId}/allowances/tokens?spender.id=eq:${smartContractId}&token.id=eq:${usdcAddresses[networkKey]}`
...
const grantAllowanceUsd = async (signerZero: DAppSigner, contractId: string, amountUsd: number): Promise<boolean> => {
  ...
  console.log(usdcAddresses[signerZero.getLedgerId().toString().toUpperCase() as keyof typeof usdcAddresses])
  console.log(contractId)
  console.log(amountUsd)
  console.log(ContractId.fromString(contractId).toEvmAddress())
  const approveTx = await new ContractExecuteTransaction()
    .setContractId(usdcAddresses[signerZero.getLedgerId().toString().toUpperCase() as keyof typeof usdcAddresses])
