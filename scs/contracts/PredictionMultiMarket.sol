...
contract PredictionMarket {
    ...
    mapping(uint256 => bool) public outcomes;               // true = YES wins, false = NO wins
    mapping(uint256 => uint256) public resolutionTimes;
    mapping(uint256 => uint256) public totalCollaterals;
    
    mapping(uint256 => mapping(address => uint256)) public yesTokens;
    mapping(uint256 => mapping(address => uint256)) public noTokens;
    
    event PositionTokensPurchased(uint256 marketId, address indexed buyer, uint256 amount, uint256 nPositionTokens, bool isSell);
    event MarketResolved(uint256 marketId, bool outcome);
    event WinningsRedeemed(uint256 marketId, address indexed user, uint256 amount);
    ...
    */
    function createNewMarket(uint256 marketId, string memory _statement) external onlyOwner {
    ...
    */
    function buyPositionTokensOnBehalfAtomic(uint256 marketId, address yes, address no, uint256 collateralUSDC, uint256 nPositionTokens) external onlyOwner {
    ...
    */
    function resolveMarket(uint256 marketId, bool noYes) external onlyOracle {
    ...
    */
    function redeem(uint256 marketId) external returns (uint256 amountUSDC) {
    ...
    */
    function getUserTokens(uint256 marketId, address user) external view returns (uint256 yes, uint256 no) {
    ...
    */
    function getTotalCollateral(uint256 marketId) external view returns (uint256 amountUSDC) {
